...
export default function NewPostPage() {
  ...
  const renderPreview = (md: string) => {
    return md
      ...
      .replace(/^> (.+)$/gm, '<blockquote class="border-l-4 border-border pl-4 text-muted-foreground italic my-4">$1</blockquote>')
          ...
          <div className="border-b border-border px-8 py-5">
            ...
            {slug && (
              <p className="mt-1.5 text-xs text-muted-foreground">
                Slug:{" "}
                <span className="font-mono text-foreground/70">/blog/{slug}</span>
              </p>
            )}
          </div>
            ...
            {coverImage ? (
              <div className="group relative overflow-hidden rounded-lg border border-border">
                ...
                <button
                  ...
                  className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-background/80 opacity-0 transition-opacity group-hover:opacity-100"
                >
